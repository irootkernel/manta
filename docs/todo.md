...
`HARDE-001` and `HARDE-002` are complete. Continue with `HARDE-003` through `HARDE-007` as separate reviewable PRs in numerical order. Each task must retain its focused evidence and pass the affected existing suites before its roadmap status changes to `Done`. `HARDE-007` remains the final end-to-end gate; passing an earlier focused test does not close the epic.
